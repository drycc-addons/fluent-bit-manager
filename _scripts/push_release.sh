...
curl -s --data-binary @dist/fluentbit_manager-${CI_COMMIT_TAG}.tar.gz -H "Content-Type: application/octet-stream" -H "${GITHUB_AUTH_HEAD}" -X POST "${GITHUB_ASSET_UPLOAD_URL}?name=fluentbit_manager-${CI_COMMIT_TAG}.tar.gz"
